...
int DFAtab[STATES][SYMBOLS]; /* a 2D array where DFAtab[i][j] represents the state to transition to from state i on input symbol j. */
 
char StateName[STATES][STATES+1];   /* state-name table */
 
int N_optDFA_states;    /* number of optimized DFA states */
int OptDFA[STATES][SYMBOLS]; /* optimized dfa transition table */
char NEW_finals[STATES+1]; /* new final states after optimization*/
